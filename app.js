...
async function initAudio() {
  if (!audioCtx) {
    ...
    convolver.buffer = createReverbBuffer(audioCtx, 3.5, 3.0); // ★リバーブ減衰時間変更

    dryGain = audioCtx.createGain();
    wetGain = audioCtx.createGain();

    dryGain.connect(masterGain);
    dryGain.connect(wetGain); // ★全体にリバーブをかけるための追加ルーティング
...
function updateReverb() {
  ...
  wetGain.gain.value = wetVal * 1.5; // ★リバーブの効きを良くするため1.5倍
    ...
    btn.addEventListener('click', async (e) => {
      ...
      if (db) {
        try {
          ...
          // ★修正：データベース保存成功時もローカルUIに即反映させる
          if (appMode === "make") simulateLocalTrack(name, `assets/sounds/${fileName}`, localId, assetId);
        } catch (err) {
...
function setupDraggableClip(clipEl, track) {
  ...
  const onStart = (e) => {
    // ★追加: スクロールなどのデフォルト動作を完全にブロック
    if (e.cancelable) e.preventDefault(); 
    
    if (!isMasterPlaying) initAudio();
    isDragging = true;
    startX = e.type.includes('mouse') ? e.clientX : (e.touches ? e.touches.clientX : e.changedTouches.clientX);
  ...
  const onMove = (e) => {
    ...
    const currentX = e.type.includes('mouse') ? e.clientX : (e.touches ? e.touches.clientX : e.changedTouches.clientX);
  ...
  const onEnd = async (e) => {
    ...
    const currentX = e.type.includes('mouse') ? e.clientX : (e.changedTouches ? e.changedTouches.clientX : e.touches.clientX);
  ...
  // ★修正: passive: false にすることで、触れた瞬間に preventDefault() が効き、すぐ掴めるようになる
...
if (btnExportMaster) {
  btnExportMaster.addEventListener('click', async () => {
    ...
    try {
      ...
      offlineDryGain.connect(offlineWetGain); // ★全体リバーブ
