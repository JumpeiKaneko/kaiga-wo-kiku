...
function resetAudioAndUI() {
  ...
  if (document.getElementById('btn-master-play-stop')) {
    document.getElementById('btn-master-play-stop').innerText = "全体の音を再生";
  ...
  if (baseAudioSource) { try{baseAudioSource.stop()}catch(e){} baseAudioSource = null; }
  if (guideAiSource) { try{guideAiSource.stop()}catch(e){} guideAiSource = null; }
  document.getElementById('btn-mikiki-base-play').innerText = "環境音をONにする";
  document.getElementById('btn-guide-base-play').innerText = "AI解説をONにする";
  document.getElementById('btn-mikiki-base-play').classList.remove('recording');
...
document.getElementById('btn-choice-mikiki-ws').addEventListener('click', (e) => {
  ...
  document.getElementById('mikiki-base-sound-section').style.display = 'block';
...
document.getElementById('btn-choice-guide-ws').addEventListener('click', (e) => {
  ...
  document.getElementById('mikiki-base-sound-section').style.display = 'none';
...
// ======= ワークショップ：上部ボタン（ベース音ON/OFF） =======
document.getElementById('btn-mikiki-base-play').addEventListener('click', async (e) => {
  await initAudio();
  if (baseAudioSource) {
    baseAudioSource.stop(); baseAudioSource = null;
    e.target.innerText = "環境音をONにする"; e.target.classList.remove('recording');
  } else {
    try {
      const res = await fetch("assets/sounds/ambient_base.mp3");
      const buf = await audioCtx.decodeAudioData(await res.arrayBuffer());
      baseAudioSource = audioCtx.createBufferSource();
      baseAudioSource.buffer = buf; baseAudioSource.loop = true;
      baseAudioSource.connect(masterGain); baseAudioSource.start(0);
      e.target.innerText = "環境音をOFFにする"; e.target.classList.add('recording');
...
document.getElementById('btn-guide-base-play').addEventListener('click', async (e) => {
  ...
  if (guideAiSource) {
    ...
    } catch(err) { alert("AI解説用ファイル(ai_guide.mp3)が見つかりません。"); }
  }
});
    ...
    exhibitGuideTracks.forEach(async (url, idx) => {
      try {
        ...
        if (r.ok) {
          ...
          g.gain.value = 0.3; // 投稿音はバックミュージックとして控えめに
          src.connect(g); g.connect(masterGain);
          src.start(audioCtx.currentTime + Math.random() * 5); // ランダムにずらして再生
...
document.getElementById('btn-master-play-stop').addEventListener('click', async (e) => {
  ...
  try {
    if (!isMasterPlaying) {
      ...
      e.target.innerText = "全体の音を停止"; e.target.classList.add('recording');
      startTime = audioCtx.currentTime; tracks.forEach(t => startTrackSource(t, 0)); updateProgress();
    } else {
      isMasterPlaying = false;
      e.target.innerText = "全体の音を再生"; e.target.classList.remove('recording');
