...
let tracks = []; let presetTracksCache = []; let isMasterPlaying = false; let startTime = 0; let animationFrameId; let isTransportBusy = false;
const PIXELS_PER_SEC = 30;

let outputAudioBuffer = null; 
let outputAudioSource = null; 
let isOutputLooping = true; // デフォルトでループ
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (arExitBtn) {
    arExitBtn.addEventListener('click', () => { stopARMode(); arExitBtn.style.display = 'none'; appExhibit.style.display = 'block'; });
  }
  ...
  if (btnCamera) {
    btnCamera.addEventListener('click', async (e) => { e.preventDefault(); appExhibit.style.display = 'none'; await startARMode(); if (arExitBtn) arExitBtn.style.display = 'block'; });
  }
            ...
            if (storage && db) {
              try {
                ...
                await db.collection("tracks").add({
                  user: currentUser, 
                  name: `録音 ${String(timestamp).substring(9, 13)}`, 
                  url: downloadUrl,
                  category: "録音データ",
                  createdAt: firebase.firestore.FieldValue.serverTimestamp()
                });
              } catch (e) { alert("録音データの保存に失敗しました。"); }
            }
  ...
  if (btnExportMaster) {
    btnExportMaster.addEventListener('click', async () => {
      ...
      try {
        ...
        if (storage && db) {
          let targetCollection = "exports";
          
          // ★ ミキキモードの場合、1回目か2回目かを選択させる
          if (appMode === "mikiki") {
            const isFirst = confirm("「1回目の作品」として投稿しますか？\n\n・[OK] を押すと 1回目 に保存\n・[キャンセル] を押すと 2回目 に保存");
            targetCollection = isFirst ? "mikiki_exports_1" : "mikiki_exports_2";
          } else {
            targetCollection = "exports";
          }
  ...
  // ★ 作品一覧表示：スマホ割れ対策（縦並び）と、1回目・2回目の分離表示
  ...
  if (btnShowWorks) {
    btnShowWorks.addEventListener('click', async () => {
      ...
      const renderItem = (doc) => {
        ...
        const delBtn = isOwn ? `<button class="action-btn gallery-delete-btn" data-id="${doc.id}" data-col="${colName}" style="color:#e74c3c; padding:6px 12px; font-size:0.7rem;">削除</button>` : '';
        
        const el = document.createElement('div');
        el.className = 'track-item';
        // ★ スマホで割れないようにフレックスボックスで縦並びに構成
        el.style.borderBottom = '1px solid var(--line-color)'; 
        el.style.padding = '15px 0';
        el.style.flexDirection = 'column';
        el.style.alignItems = 'stretch';
        
        // ★ タイトルも word-break: break-all を指定して押し出しを防止
        el.innerHTML = `
          <div style="display:flex; flex-direction:column; gap:4px; margin-bottom:10px;">
            <div class="track-name" style="font-size:0.85rem; color:var(--text-main); font-weight:bold; word-break:break-all;">${data.title || 'Untitled'}</div>
      ...
      if (appMode === "mikiki") {
        // ▼ 1回目のリスト取得
        worksListContainer.insertAdjacentHTML('beforeend', '<div style="font-size:0.9rem; font-weight:bold; color:var(--text-main); margin-bottom:10px; margin-top:10px; border-bottom:2px solid var(--text-main); padding-bottom:5px;">▼ 1回目 完成作品</div>');
        const snap1 = await db.collection("mikiki_exports_1").orderBy("createdAt", "desc").get();
        if (snap1.empty) worksListContainer.insertAdjacentHTML('beforeend', '<div style="font-size:0.75rem; color:var(--text-muted); margin-bottom:20px;">まだ作品がありません。</div>');
        else snap1.forEach(doc => renderItem(doc));

        // ▼ 2回目のリスト取得
        worksListContainer.insertAdjacentHTML('beforeend', '<div style="font-size:0.9rem; font-weight:bold; color:var(--text-main); margin-bottom:10px; margin-top:30px; border-bottom:2px solid var(--text-main); padding-bottom:5px;">▼ 2回目 完成作品</div>');
        const snap2 = await db.collection("mikiki_exports_2").orderBy("createdAt", "desc").get();
        if (snap2.empty) worksListContainer.insertAdjacentHTML('beforeend', '<div style="font-size:0.75rem; color:var(--text-muted); margin-bottom:20px;">まだ作品がありません。</div>');
        else snap2.forEach(doc => renderItem(doc));
        
      } else {
        const snap = await db.collection("exports").orderBy("createdAt", "desc").get();
        if (snap.empty) { worksListContainer.innerHTML = '<div style="font-size: 0.8rem;">まだ作品がありません。</div>'; }
        else { snap.forEach(doc => renderItem(doc)); }
      }
      ...
      document.querySelectorAll('.gallery-delete-btn').forEach(b => {
        b.addEventListener('click', async (e) => {
          ...
          await db.collection(targetCol).doc(e.target.getAttribute('data-id')).delete();
  ...
  // ★ 「聴く」モードでの再生：1回目と2回目を両方取得して混ぜて再生する
...
async function startARMode() {
  ...
  if (!window.arEventsBound) {
    for (let i = 0; i < 5; i++) {
      const targetEl = document.getElementById(`target-${i}`);
      if (targetEl) {
        targetEl.addEventListener('targetFound', () => { if (AR_WORKS[i]) { AR_WORKS[i].targetVolume = 1.0; if (AR_WORKS[i].gainNode) AR_WORKS[i].gainNode.gain.value = 1.0; } });
        targetEl.addEventListener('targetLost', () => { if (AR_WORKS[i]) { AR_WORKS[i].targetVolume = 0.0; } });
      }
    }
  ...
  if (sceneEl) {
    ...
    sceneEl.addEventListener('arError', () => { document.getElementById('ar-loading').style.display = 'none'; alert("カメラ起動失敗"); });
...
async function startSyncTracks() {
  ...
  if (presetTracksCache.length === 0) {
    const loadInitialAssets = ASSETS.map(async (asset) => {
      ...
      return {
        ...
        gainNode: trackGain, reverbGainNode: trackRevGain, isLooping: true, volume: 1.0, isActive: false,
    ...
    presetTracksCache = await Promise.all(loadInitialAssets);
  }
  ...
  if (db && currentUser) {
    unsubscribeTracks = db.collection("tracks").where("user", "==", currentUser).onSnapshot(async (snapshot) => {
      let userTracksData = [];
      snapshot.forEach(doc => { userTracksData.push({ id: doc.id, ...doc.data() }); });
      userTracksData.sort((a, b) => (a.createdAt?.toMillis() || 0) - (b.createdAt?.toMillis() || 0));

      const loadPromises = userTracksData.map(async (data) => {
        const existingTrack = tracks.find(t => t.dbDocId === data.id);
        ...
        return {
          id: data.id, dbDocId: data.id, name: data.name, url: safeUrl, buffer: audioBuffer, source: null, previewSource: null,
          gainNode: trackGain, reverbGainNode: trackRevGain, isLooping: true, volume: 1.0, isActive: false,
      ...
      const loadedUserTracks = await Promise.all(loadPromises);
      tracks = [...presetTracksCache, ...loadedUserTracks.filter(t => t)];
      
      if (emptyMsg) emptyMsg.style.display = 'none';
      renderUI();
    });
  } else {
    tracks = [...presetTracksCache];
    if (emptyMsg) emptyMsg.style.display = 'none';
    renderUI();
  }
}
...
function renderUI() {
  ...
  tracks.forEach((track) => {
    ...
    // ★スマホ割れ対策（Flexboxのオーバーフロー制御）
    const nameTrackHTML = `<div style="display:flex; flex-direction:column; flex-grow:1; min-width:0; margin-left:10px; overflow:hidden;"><span class="track-name-label" style="font-weight:bold; color:var(--text-main); font-size:0.8rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${track.name}</span>${subtitleHTML}</div>`;
    
    const playBtnHTML = `<button class="action-btn preview-btn" data-id="${track.dbDocId}">再生</button>`;
    const detailBtnHTML = `<button class="action-btn toggle-detail-btn" data-id="${track.dbDocId}" style="background:transparent; border:none; color:var(--text-muted); text-decoration:underline; font-size:0.65rem; flex-shrink:0;">▼ 詳細</button>`;
    ...
    mixerEl.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; width:100%; flex-wrap:nowrap;">
        <div style="display:flex; align-items:center; flex-grow:1; min-width:0;">${onOffBtnHTML}${nameTrackHTML}</div>
...
async function startListenEveryone() {
  if (db) {
    try {
      // ★ 1回目と2回目のエクスポートを両方読み込んで混ぜる
      const snap1 = await db.collection("mikiki_exports_1").get();
      const snap2 = await db.collection("mikiki_exports_2").get();
      everyoneTracks = [];
      snap1.forEach(doc => { const data = doc.data(); if(data.url) everyoneTracks.push(data); });
      snap2.forEach(doc => { const data = doc.data(); if(data.url) everyoneTracks.push(data); });
