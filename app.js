...
function playUnityAudio() { const instance = getUnityInstance(); if (instance) instance.SendMessage('AudioController', 'PlayBackgroundSound'); }
function stopUnityAudio() { const instance = getUnityInstance(); if (instance) instance.SendMessage('AudioController', 'StopBackgroundSound'); }
function loadUnityInstance() {
    ...
    var config = {
        ...
        companyName: "DefaultCompany", productName: "kaiga-wo-kiku", productVersion: "0.1",
    };
    var script = document.createElement("script");
    script.src = loaderUrl;
    script.onload = () => { createUnityInstance(document.querySelector("#unity-canvas"), config, (p) => {}).then((i) => { window.unityInstance = i; }); };
...
if (btnPlayUnityAudio) {
    btnPlayUnityAudio.addEventListener('click', () => {
        if (appMode === "mikiki") {
            if (!getUnityInstance()) { alert("Unityシステムをロード中です。"); return; }
            if (!isListenModePlaying) { playUnityAudio(); isListenModePlaying = true; btnPlayUnityAudio.innerText = "絵画の音を停止"; btnPlayUnityAudio.classList.add('recording'); }
            else { stopUnityAudio(); isListenModePlaying = false; btnPlayUnityAudio.innerText = "絵画 the 音を聴く"; btnPlayUnityAudio.classList.remove('recording'); }
        } else {
            if (!isListenModePlaying) {
                if (tracks.length > 0 && tracks[0].url) {
                    currentGalleryAudio = new Audio(formalizeUrl(tracks[0].url)); currentGalleryAudio.loop = true; currentGalleryAudio.play(); isListenModePlaying = true; btnPlayUnityAudio.innerText = "絵画の音を停止";
                }
            } else { if (currentGalleryAudio) { currentGalleryAudio.pause(); currentGalleryAudio = null; } isListenModePlaying = false; btnPlayUnityAudio.innerText = "絵画の音を聴く"; }
        }
    });
}
            ...
            try {
                ...
                mediaRecorder.onstop = async () => {
                    ...
                    await db.collection("tracks").add({
                        ...
                        storagePath: storagePath, isLooping: true, volume: 1.0, delayTime: 0, estimatedDuration: (Date.now() - recordStart) / 1000,
                        createdAt: firebase.firestore.FieldValue.serverTimestamp()
                    });
                    btnRecord.innerText = "録音を開始";
                };
                const recordStart = Date.now(); mediaRecorder.start(); isRecording = true;
                btnRecord.innerText = "録音を停止"; btnRecord.classList.add('recording'); playUnityAudio();
            } catch (err) { alert("マイクへのアクセスが拒否されました。"); }
        } else { mediaRecorder.stop(); mediaRecorder.stream.getTracks().forEach(t => t.stop()); isRecording = false; btnRecord.classList.remove('recording'); stopUnityAudio(); }
    });
}
...
async function loadGalleryWorks() {
    try {
        ...
        if (snapshot.empty) { worksListContainer.innerHTML = '<div style="font-size:0.75rem; color:var(--text-muted);">まだ作品がありません。</div>'; return; }
        
        snapshot.forEach(doc => {
            const data = doc.data(); const itemEl = document.createElement('div'); itemEl.className = 'track-item'; itemEl.style.borderBottom = '1px solid var(--line-color)'; itemEl.style.padding = '12px 0';
            const isOwn = (data.user === currentUser); const delBtnHTML = isOwn ? `<button class="action-btn gallery-delete-btn" data-id="${doc.id}" style="color:var(--danger); margin-left:12px;">削除</button>` : '';
            itemEl.innerHTML = `<div style="display:flex; flex-direction:column; gap:4px; max-width:60%;"><div class="track-name" style="font-size:0.75rem; color:var(--text-main);">${data.title || 'Untitled'}</div><div style="font-size:0.55rem; color:var(--text-muted);">by ${data.user}</div></div><div class="track-controls" style="flex-grow:0; gap: 0;"><button class="action-btn gallery-play-btn" data-url="${data.url}">再生</button>${delBtnHTML}</div>`;
        ...
        document.querySelectorAll('.gallery-play-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (currentGalleryPlayBtn === e.target) { if (currentGalleryAudio) { currentGalleryAudio.pause(); currentGalleryAudio = null; } e.target.innerText = '再生'; currentGalleryPlayBtn = null; return; }
                if (currentGalleryAudio) { currentGalleryAudio.pause(); if (currentGalleryPlayBtn) currentGalleryPlayBtn.innerText = '再生'; }
                currentGalleryAudio = new Audio(formalizeUrl(url)); currentGalleryAudio.loop = true; currentGalleryAudio.play(); currentGalleryPlayBtn = e.target; e.target.innerText = '停止';
            });
        });
        ...
        document.querySelectorAll('.gallery-delete-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                const targetCollection = (appMode === "make") ? "make_exports" : "exports";
                await db.collection(targetCollection).doc(e.target.getAttribute('data-id')).delete(); loadGalleryWorks();
            });
        });
    } catch (err) { console.error(err); }
}

if (btnCloseWorks) { btnCloseWorks.addEventListener('click', () => { worksModal.style.display = 'none'; if (currentGalleryAudio) { currentGalleryAudio.pause(); currentGalleryAudio = null; } if (currentGalleryPlayBtn) currentGalleryPlayBtn.innerText = '再生'; currentGalleryPlayBtn = null; }); }

// --- Web Audio API コア初期化 ---
async function initAudio() {
    if (!audioCtx) {
        audioCtx = new (window.AudioContext || window.webkitAudioContext)(); masterGain = audioCtx.createGain(); masterGain.gain.value = 1.0; masterGain.connect(audioCtx.destination);
        convolver = audioCtx.createConvolver(); convolver.buffer = createReverbBuffer(audioCtx, 3.0, 2.0); dryGain = audioCtx.createGain(); wetGain = audioCtx.createGain();
        dryGain.connect(masterGain); wetGain.connect(convolver); convolver.connect(masterGain); updateReverb();
    }
...
function createReverbBuffer(ctx, duration, decay) {
    ...
    for (let channel = 0; channel < 2; channel++) { const data = impulse.getChannelData(channel); for (let i = 0; i < length; i++) data[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / length, decay); }
    return impulse;
}
function updateReverb() { if (!dryGain || !wetGain || !reverbSlider) return; const wetVal = parseFloat(reverbSlider.value); wetGain.gain.value = wetVal; dryGain.gain.value = 1.0 - (wetVal * 0.5); }
...
function startSyncTracks() {
    if (appMode === "make") {
            ...
            const loadPromises = MAKE_MODE_ASSETS.map(async (asset) => {
                ...
                if (!doc) {
                    await db.collection("make_tracks").add({
                        user: currentUser, assetId: asset.id, name: asset.name,
                        url: `./assets/sounds/${asset.fileName}`,
                        delayTime: 0, volume: 1.0, isLooping: true, createdAt: firebase.firestore.FieldValue.serverTimestamp()
                    });
                }
                ...
                return {
                    id: asset.id, dbDocId: doc ? doc.id : null, name: asset.name, url: trackUrl, buffer: audioBuffer, source: null,
                    gainNode: audioCtx ? audioCtx.createGain() : null, isLooping: cloudData.isLooping, volume: cloudData.volume, delayTime: cloudData.delayTime, duration: audioBuffer ? audioBuffer.duration : 5
                };
            });
        ...
        db.collection("tracks").where("user", "==", currentUser).onSnapshot(async (snapshot) => {
            if (snapshot.empty) { if(emptyMsg) emptyMsg.style.display = 'block'; if(trackListEl) trackListEl.innerHTML = ''; if(timelineTracksEl) timelineTracksEl.innerHTML = ''; tracks = []; return; }
            if (emptyMsg) emptyMsg.style.display = 'none';
            ...
            const loadPromises = snapshot.docs.map(async (docSnapshot) => {
                ...
                if (existingTrack) {
                    existingTrack.name = data.name; existingTrack.isLooping = data.isLooping; existingTrack.volume = data.volume;
                    if (existingTrack.delayTime !== data.delayTime) { existingTrack.delayTime = data.delayTime; if (isMasterPlaying) startTrackSource(existingTrack, audioCtx.currentTime - startTime); }
                    if (existingTrack.gainNode) existingTrack.gainNode.gain.value = existingTrack.volume; return existingTrack;
                }
                let audioBuffer = null;
                ...
                const newTrack = { id: id, dbDocId: id, name: data.name, url: safeUrl, buffer: audioBuffer, source: null, gainNode: audioCtx ? audioCtx.createGain() : null, isLooping: true, volume: data.volume, delayTime: data.delayTime, duration: audioBuffer ? audioBuffer.duration : 5 };
                if (audioCtx && newTrack.gainNode) { newTrack.gainNode.connect(dryGain); newTrack.gainNode.connect(wetGain); newTrack.gainNode.gain.value = newTrack.volume; }
                return newTrack;
            });
            tracks = await Promise.all(loadPromises);
            renderUI();
...
function renderUI() {
    ...
    tracks.forEach((track) => {
        ...
        mixerEl.innerHTML = `${nameTrackHTML}<div class="track-controls"><button class="action-btn loop-btn ${track.isLooping ? 'active' : ''}" data-id="${track.id}">Loop: ${track.isLooping ? 'ON' : 'OFF'}</button><div class="vol-slider-wrapper"><input type="range" class="vol-slider" data-id="${track.id}" min="0" max="1" step="0.01" value="${track.volume}"></div>${actionButtonsHTML}</div>`;
        trackListEl.appendChild(mixerEl);

        const rowEl = document.createElement('div'); rowEl.className = 'timeline-row';
        const clipEl = document.createElement('div'); clipEl.className = 'timeline-clip'; clipEl.innerText = track.name + (track.isLooping ? " ↻" : "");
        const leftPx = track.delayTime * PIXELS_PER_SEC; clipEl.style.left = `${leftPx}px`;
        
        if (track.isLooping) { clipEl.style.width = `1200px`; clipEl.style.background = "repeating-linear-gradient(90deg, #f0f0f0, #f0f0f0 100px, #e8e8e8 101px)"; }
        else { clipEl.style.width = `${Math.max(track.duration * PIXELS_PER_SEC, 20)}px`; }
        if (leftPx + (track.duration * PIXELS_PER_SEC) > maxTimelineWidth) maxTimelineWidth = leftPx + (track.duration * PIXELS_PER_SEC) + 300;

        setupDraggableClip(clipEl, track); rowEl.appendChild(clipEl); timelineTracksEl.appendChild(rowEl);
    });
...
function setupDraggableClip(clipEl, track) {
    ...
    const onStart = (e) => { if (!isMasterPlaying) initAudio(); isDragging = true; startX = e.type.includes('mouse') ? e.clientX : e.touches[0].clientX; initialDelay = track.delayTime; clipEl.style.zIndex = 100; document.body.style.userSelect = 'none'; };
    const onMove = (e) => { if (!isDragging) return; const currentX = e.type.includes('mouse') ? e.clientX : e.touches[0].clientX; clipEl.style.left = `${Math.max(0, initialDelay + ((currentX - startX) / PIXELS_PER_SEC)) * PIXELS_PER_SEC}px`; };
    const onEnd = async (e) => {
        if (!isDragging) return; isDragging = false; clipEl.style.zIndex = ''; document.body.style.userSelect = '';
        const currentX = e.type.includes('mouse') ? e.clientX : e.changedTouches[0].clientX; let newDelay = Math.max(0, initialDelay + ((currentX - startX) / PIXELS_PER_SEC));
        const targetCollection = (appMode === "make") ? "make_tracks" : "tracks";
        if (track.dbDocId) await db.collection(targetCollection).doc(track.dbDocId).update({ delayTime: newDelay });
    };
    clipEl.addEventListener('mousedown', onStart); clipEl.addEventListener('touchstart', onStart, {passive: true});
...
function attachMixerEvents() {
    document.querySelectorAll('.track-name-input').forEach(input => { input.addEventListener('change', async e => { await db.collection("tracks").doc(e.target.getAttribute('data-id')).update({ name: e.target.value.trim() }); }); });
    document.querySelectorAll('.loop-btn').forEach(btn => {
        btn.addEventListener('click', async e => {
            const id = e.target.getAttribute('data-id'); const t = tracks.find(x => x.id === id); if(!t) return;
            const targetCollection = (appMode === "make") ? "make_tracks" : "tracks";
    ...
    document.querySelectorAll('.vol-slider').forEach(slider => {
        slider.addEventListener('input', e => { const t = tracks.find(x => x.id === e.target.getAttribute('data-id')); if (t && t.gainNode) t.gainNode.gain.value = parseFloat(e.target.value); });
        slider.addEventListener('change', async e => {
            const t = tracks.find(x => x.id === e.target.getAttribute('data-id')); if(!t) return;
            const targetCollection = (appMode === "make") ? "make_tracks" : "tracks";
            if (t.dbDocId) await db.collection(targetCollection).doc(t.dbDocId).update({ volume: parseFloat(e.target.value) });
        });
    });
    document.querySelectorAll('.clone-btn').forEach(btn => { btn.addEventListener('click', async e => { const t = tracks.find(x => x.id === e.target.getAttribute('data-id')); if (!t) return; await db.collection("tracks").add({ user: currentUser, name: `${t.name} c`, url: t.url, storagePath: t.storagePath || "", isLooping: t.isLooping, volume: t.volume, delayTime: t.delayTime, estimatedDuration: t.duration, createdAt: firebase.firestore.FieldValue.serverTimestamp() }); }); });
    document.querySelectorAll('.delete-btn').forEach(btn => { btn.addEventListener('click', async e => { if(confirm("削除しますか？")) await db.collection("tracks").doc(e.target.getAttribute('data-id')).delete(); }); });
}
...
function startTrackSource(track, elapsed = 0) {
    ...
    if (isMasterPlaying) {
        if (now < targetStartTime) { track.source.start(targetStartTime); } 
        else { const offset = now - targetStartTime; if (track.isLooping) { track.source.start(0, offset % track.buffer.duration); } else if (offset < track.buffer.duration) { track.source.start(0, offset); } }
    }
}
...
if (btnPlay) {
    btnPlay.addEventListener('click', async () => {
        ...
        try { await initAudio(); isMasterPlaying = true; btnPlay.classList.add('active'); if (btnStop) btnStop.classList.remove('active'); startTime = audioCtx.currentTime; tracks.forEach(t => startTrackSource(t, 0)); updateProgress(); } 
        finally { isTransportBusy = false; }
    });
}
if (btnStop) { btnStop.addEventListener('click', () => { isMasterPlaying = false; btnPlay.classList.remove('active'); btnStop.classList.add('active'); tracks.forEach(t => { if (t.source) { try{t.source.stop()}catch(e){} t.source = null; } }); cancelAnimationFrame(animationFrameId); if (playheadEl) playheadEl.style.left = '0px'; }); }
if (btnRewind) { btnRewind.addEventListener('click', () => { const wasPlaying = isMasterPlaying; isMasterPlaying = false; tracks.forEach(t => { if (t.source) { try{t.source.stop()}catch(e){} t.source = null; } }); cancelAnimationFrame(animationFrameId); if (btnPlay) btnPlay.classList.remove('active'); if (btnStop) btnStop.classList.remove('active'); if (playheadEl) playheadEl.style.left = '0px'; if (audioCtx) startTime = audioCtx.currentTime; if (wasPlaying) setTimeout(() => { btnPlay.click(); }, 100); }); }
if (btnMasterLoop) { btnMasterLoop.addEventListener('click', () => { isMasterLooping = !isMasterLooping; btnMasterLoop.classList.toggle('active', isMasterLooping); btnMasterLoop.innerText = `全体ループ: ${isMasterLooping ? 'ON' : 'OFF'}`; }); }

function updateProgress() {
    animationFrameId = requestAnimationFrame(updateProgress); if (!isMasterPlaying) return;
    const elapsed = audioCtx.currentTime - startTime; if (playheadEl) playheadEl.style.left = `${elapsed * PIXELS_PER_SEC}px`;
    ...
    if (maxDur > 0 && elapsed >= maxDur) {
        if (isMasterLooping) { startTime = audioCtx.currentTime; tracks.forEach(t => { if (t.source) { try{t.source.stop()}catch(e){} t.source = null; } startTrackSource(t, 0); }); } 
        else { if (btnStop) btnStop.click(); }
    }
}
...
function checkExistingExport() {
    ...
    db.collection(targetCollection).doc(currentUser).onSnapshot((doc) => {
        if (doc.exists) {
            const data = doc.data(); if (inputExportName) inputExportName.value = data.title || ""; if (outputFileDisplay) outputFileDisplay.innerText = data.title || 'Master Track';
            fetchExistingExportBuffer(data.url);
        }
    });
}
async function fetchExistingExportBuffer(url) { try { await initAudio(); const response = await fetch(formalizeUrl(url)); outputAudioBuffer = await audioCtx.decodeAudioData(await response.arrayBuffer()); if (outputPlayerContainer) outputPlayerContainer.style.display = 'block'; } catch(e) {} }

if (btnExportMaster) {
    btnExportMaster.addEventListener('click', async () => {
        if (tracks.length === 0) return; const exportName = inputExportName.value.trim() || `Master_${currentUser}`;
        btnExportMaster.innerText = "音源を合成中..."; btnExportMaster.disabled = true;
        try {
            await initAudio(); const maxDur = Math.max(...tracks.map(t => parseFloat(t.duration) + parseFloat(t.delayTime)));
            const offlineCtx = new OfflineAudioContext(2, audioCtx.sampleRate * maxDur, audioCtx.sampleRate);
            const offlineMasterGain = offlineCtx.createGain(); offlineMasterGain.connect(offlineCtx.destination);
            tracks.forEach(t => { if (!t.buffer) return; const source = offlineCtx.createBufferSource(); source.buffer = t.buffer; source.loop = t.isLooping; const gain = offlineCtx.createGain(); gain.gain.value = t.volume; source.connect(gain); gain.connect(offlineMasterGain); source.start(t.delayTime); });
            
            const renderedBuffer = await offlineCtx.startRendering(); const wavBlob = bufferToWavBlob(renderedBuffer);
            const targetCollection = (appMode === "make") ? "make_exports" : "exports";
            const storagePath = `${targetCollection}/${exportName}_${Date.now()}.mp3`;
            const snapshot = await storage.ref().child(storagePath).put(wavBlob); const downloadUrl = await snapshot.ref.getDownloadURL();
