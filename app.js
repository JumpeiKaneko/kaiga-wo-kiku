...
// 再⽣ボタンとタイムラインをスクロール追従（固定表⽰）にする処理
...
function resetAudioAndUI() {
  ...
  if (appMode === "mikiki" && isListenModePlaying) {
    ...
    if (btnPlayUnityAudio) {
      btnPlayUnityAudio.innerText = "絵画の⾳を聴く";
...
if (btnLogin) {
  btnLogin.addEventListener('click', async (e) => {
    ...
    if (!username) { alert("ユーザー名を⼊⼒してください。"); return; }
    if (db && loginStatus === "first") {
      try {
        const userDoc = await db.collection("users").doc(username).get();
        if (userDoc.exists) {
          alert("このユーザー名は既に存在します。別の名前を⼊⼒するか、戻って「2回⽬以降」を選択してください。"); return;
        }
...
if (btnChoiceMikiki) {
  btnChoiceMikiki.addEventListener('click', async (e) => {
    ...
    if (btnModeListen) btnModeListen.innerText = "展⽰";
...
async function startMikikiMode() {
  ...
  try {
    if (!navigator.bluetooth || !navigator.bluetooth.requestLEScan) {
      throw new Error("お使いのブラウザはWeb Bluetoothのスキャンに対応していません。（iOSは⾮対応です）");
    }
  ...
  btnPlayUnityAudio.addEventListener('click', async () => {
    if (appMode === "mikiki") {
      ...
      if (!isListenModePlaying) {
        ...
        btnPlayUnityAudio.innerText = "展⽰の⾳を停⽌";
        btnPlayUnityAudio.classList.add('recording');
      } else {
        stopMikikiMode();
        isListenModePlaying = false;
        btnPlayUnityAudio.innerText = "展⽰の⾳を聴く";
        btnPlayUnityAudio.classList.remove('recording');
      }
    } else {
      if (!isListenModePlaying) {
        if (tracks.length > 0 && tracks.url) {
          ...
          isListenModePlaying = true;
          btnPlayUnityAudio.innerText = "絵画の⾳を停⽌";
        }
      } else {
        if (currentGalleryAudio) { currentGalleryAudio.pause(); currentGalleryAudio = null; }
        isListenModePlaying = false;
...
// ★修正：スマホ割れ対策（縦並びレイアウト）と、ミキキモードの1回目・2回目の分割表示
    ...
    document.querySelectorAll('.gallery-play-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (currentGalleryPlayBtn === e.target) {
          if (currentGalleryAudio) { currentGalleryAudio.pause(); currentGalleryAudio = null; }
          e.target.innerText = '再⽣'; currentGalleryPlayBtn = null; return;
        }
        if (currentGalleryAudio) { currentGalleryAudio.pause(); if (currentGalleryPlayBtn) currentGalleryPlayBtn.innerText = '再⽣'; }
        currentGalleryAudio = new Audio(formalizeUrl(url));
        currentGalleryAudio.loop = true; currentGalleryAudio.play();
        currentGalleryPlayBtn = e.target; e.target.innerText = '停⽌';
      });
    });
...
if (btnCloseWorks) {
  btnCloseWorks.addEventListener('click', () => {
    ...
    if (currentGalleryAudio) { currentGalleryAudio.pause(); currentGalleryAudio = null; }
    if (currentGalleryPlayBtn) { currentGalleryPlayBtn.innerText = '再⽣'; currentGalleryPlayBtn = null; }
  });
}
  ...
  if (appMode === "make" && document.getElementById('asset-grid-container')) {
    ...
    document.querySelectorAll('.asset-preview-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        ...
        assetPreviewBtn = e.target; e.target.innerText = "停⽌";
...
function setupMikikiExportUI() {
  ...
  exportSection.insertAdjacentHTML('beforeend', `
    <div style="margin-bottom: 24px;">
      <div style="font-size: 0.75rem; color: var(--text-main); margin-bottom: 8px; letter-spacing: 0.1em;">▼ 1回⽬</div>
      <button id="btn-export-master-1" class="record-btn-export">1回⽬を完成させる</button>
      <div id="output-player-container-1" style="display: none; margin-top: 15px; padding-bottom: 15px; border-bottom: 1px dashed var(--line-color);">
        <div class="track-item" style="flex-direction: row; align-items: center; justify-content: space-between; padding: 0;">
          ...
          <div class="track-controls" style="gap: 15px;">
            ...
            <button id="btn-output-play-1" class="action-btn" style="letter-spacing: 0.1em;">再⽣</button>
            <button id="btn-output-stop-1" class="action-btn" style="letter-spacing: 0.1em;">停⽌</button>
    ...
    <div>
      <div style="font-size: 0.75rem; color: var(--text-main); margin-bottom: 8px; letter-spacing: 0.1em;">▼ 2回⽬</div>
      <button id="btn-export-master-2" class="record-btn-export">2回⽬を完成させる</button>
      <div id="output-player-container-2" style="display: none; margin-top: 15px; padding-bottom: 15px; border-bottom: 1px dashed var(--line-color);">
        <div class="track-item" style="flex-direction: row; align-items: center; justify-content: space-between; padding: 0;">
          ...
          <div class="track-controls" style="gap: 15px;">
            ...
            <button id="btn-output-play-2" class="action-btn" style="letter-spacing: 0.1em;">再⽣</button>
            <button id="btn-output-stop-2" class="action-btn" style="letter-spacing: 0.1em;">停⽌</button>
  ...
  btnExport.addEventListener('click', async () => {
    ...
    try {
      ...
      if (db) {
        ...
        alert(`クラウドへの保存(${phase}回⽬)が完了しました。`);
      } else {
      ...
      btnExport.innerText = `${phase}回⽬を完成させる`;
...
if (btnExportMaster) {
  btnExportMaster.addEventListener('click', async () => {
    ...
    btnExportMaster.innerText = "⾳源を合成中...";
    ...
    try {
      ...
      console.error(err);
      alert("合成に失敗しました。スマホのメモリ不⾜か、ブラウザが未対応の可能性があります。");
    } finally {
