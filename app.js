...
const btnChoiceExhibitBeacon = document.getElementById('btn-choice-exhibit-beacon');
const btnChoiceExhibitCamera = document.getElementById('btn-choice-exhibit-camera');
...
if (btnBackToStep3FromMikiki) {
  btnBackToStep3FromMikiki.addEventListener('click', () => {
    modalStepMikikiChoice.style.display = 'none';
    modalStep3.style.display = 'block';
  });
}
...
if (btnChoiceMikiki) {
  btnChoiceMikiki.addEventListener('click', async (e) => {
    ...
    modalStep3.style.display = 'none';
    modalStepMikikiChoice.style.display = 'block';
  });
}

// ★ミキキの分岐
if (btnChoiceExhibitBeacon) {
  btnChoiceExhibitBeacon.addEventListener('click', (e) => {
    e.preventDefault();
    appMode = "mikiki";
    isCameraMode = false;
    modalStepMikikiChoice.style.display = 'none';
...
if (btnChoiceExhibitCamera) {
  btnChoiceExhibitCamera.addEventListener('click', (e) => {
    e.preventDefault();
    appMode = "mikiki";
    isCameraMode = true;
    modalStepMikikiChoice.style.display = 'none';
...
async function startMikikiMode() {
  ...
  if (isCameraMode) {
    ...
    try {
      ...
      console.error(error);
      alert("Bluetoothスキャンを開始できませんでした。");
    }
  }
...
function handleBeaconAdvertisement(event) {
  ...
  if (work) {
    ...
    const maxRssi = -50;
    let targetVol = 0;
    if (rssi >= maxRssi) targetVol = 1.0;
    else if (rssi <= minRssi) targetVol = 0.0;
    else targetVol = (rssi - minRssi) / (maxRssi - minRssi);
...
function scanColorsLoop() {
  ...
  if (videoEl.readyState === videoEl.HAVE_ENOUGH_DATA) {
    ...
    MIKIKI_WORKS.forEach(work => {
      const count = counts[work.id];
      work.targetVolume = count > 30 ? Math.min((count - 30) / 100, 1.0) : 0.0;
      work.lastSeen = Date.now();
    });
  }
...
// ★修正: ローカルで追加時、ミキサーの一番上に表示させる (unshift)
...
// ★修正: 取得したトラックを常に新しい順（一番上が最新）に並べる
    ...
    unsubscribeTracks = db.collection("tracks").where("user", "==", currentUser).onSnapshot(async (snapshot) => {
      ...
      const loadPromises = snapshot.docs.map(async (docSnapshot) => {
        ...
        return {
          ...
          gainNode: trackGain, reverbGainNode: trackRevGain, isLooping: data.isLooping !== undefined ? data.isLooping : false, isActive: isActiveState,
          volume: data.volume !== undefined ? data.volume : 1.0, trackReverb: data.trackReverb || 0.0,
          delayTime: data.delayTime !== undefined ? data.delayTime : 0, duration: audioBuffer ? audioBuffer.duration : 5, isDeletable: true
        };
      });
...
function renderUI() {
  ...
  tracks.forEach((track, index) => {
    ...
    mixerEl.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; width:100%; margin-bottom:8px;">
        <div style="display:flex; align-items:center; gap:8px;">${onOffBtnHTML}${nameTrackHTML}</div>
    ...
    if (!track.isActive) rowEl.style.display = 'none';
  ...
  // ★修正: ループのON/OFFが即座に反映されるようにバグを修正
  document.querySelectorAll('.loop-btn').forEach(btn => {
    btn.addEventListener('click', async e => {
      ...
      if (t.source) t.source.loop = t.isLooping; // 即時反映
      const targetCollection = (appMode === "make") ? "make_tracks" : "tracks";
      if (db && !dbDocId.startsWith("local_")) {
...
function startTrackSource(track, elapsed = 0) {
  ...
  if (track.reverbGainNode) { track.source.connect(track.reverbGainNode); }
...
function bindMikikiExportEvents(phase) {
  ...
  btnExport.addEventListener('click', async () => {
    ...
    try {
      ...
      if (db) {
        ...
        if(phase === 1) outputAudioBuffer1 = renderedBuffer; else outputAudioBuffer2 = renderedBuffer;
        if (outContainer) outContainer.style.display = 'block';
        if (outFileName) outFileName.innerText = exportName;
        alert("ローカル環境での合成保存が完了しました。");
      }
    } catch (err) { console.error(err); alert("合成に失敗しました。"); } finally { btnExport.innerText = `${phase}回目を完成させる`; btnExport.disabled = false; }
  });

  btnLoop.addEventListener('click', () => {
    if(phase === 1) { isOutputLooping1 = !isOutputLooping1; btnLoop.innerText = `Loop: ${isOutputLooping1 ? 'ON' : 'OFF'}`; btnLoop.classList.toggle('active', isOutputLooping1); if (outputAudioSource1) outputAudioSource1.loop = isOutputLooping1; }
    else { isOutputLooping2 = !isOutputLooping2; btnLoop.innerText = `Loop: ${isOutputLooping2 ? 'ON' : 'OFF'}`; btnLoop.classList.toggle('active', isOutputLooping2); if (outputAudioSource2) outputAudioSource2.loop = isOutputLooping2; }
  });
  ...
  btnPlay.addEventListener('click', () => {
    ...
    source.start(0);
    if(phase === 1) outputAudioSource1 = source; else outputAudioSource2 = source;
...
async function fetchExistingExportBufferMikiki(url, phase) {
  try {
    ...
    const outContainer = document.getElementById(`output-player-container-${phase}`);
    if (phase === 1) outputAudioBuffer1 = buffer; else outputAudioBuffer2 = buffer;
    if (outContainer) outContainer.style.display = 'block';
  } catch(e) {}
}
...
if (btnExportMaster) {
  btnExportMaster.addEventListener('click', async () => {
    ...
    } catch (err) { console.error(err); alert("合成に失敗しました。"); } finally { btnExportMaster.innerText = "作品を完成させる"; btnExportMaster.disabled = false; }
  });
}
